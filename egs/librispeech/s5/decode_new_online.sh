...
if [ $option -eq 1 ]; then
    ...
    online2-wav-gmm-latgen-faster --capture-device="plughw:CARD=M,DEV=0" --do-endpointing=$do_endpointing --endpoint.rule2.max-relative-cost=0.5 --endpoint.rule2.min-trailing-silence=0.5 --endpoint.rule2.min-utterance-length=0.2 --endpoint.rule2.must-contain-nonsilence=true --config=$srcdir/conf/online_decoding.conf --max-active=$max_active --beam=$beam --lattice-beam=$lattice_beam --acoustic-scale=$acwt --word-symbol-table=$graphdir/words.txt $graphdir/HCLG.fst $spk2utt_rspecifier "$wav_rspecifier" "ark,t:one-best.tra"
